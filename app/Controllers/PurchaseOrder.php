...
class PurchaseOrder extends BaseController
{
    ...
    public function choosepo()
    {
        session()->remove('success');
        if (!$this->validate([
            'origin_country' => 'required|alpha_numeric_space|min_length[3]|max_length[60]',
            'po_remarks' => 'required|alpha_numeric_punct|min_length[3]|max_length[60]',
            ...
            return redirect()->to(base_url('purchaseorder/add/' . $rqnuniq . '/' . $post_stat . '/' . $del_poline))->withInput();
        } else {
                            ...
                            if ($sender['OFFLINESTAT'] == 0) {
                                ...
                                foreach ($notiftouser_data as $sendto_user) :
                                    $var_email = array(
                                        ...
                                        'EMAIL1CUST' => $get_po_data['EMAIL1CUST'],
    ...
    public function update_action()
    {
        ...
        if (!$this->validate([
            ...
            session()->set('success', '-1');
            return redirect()->to(base_url('/purchaseorder'))->withInput();
            session()->remove('success');
        } else {
            $id_so = $this->request->getPost('csruniq');
